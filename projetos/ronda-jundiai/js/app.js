/* ================================================================
   RONDA CALLINK JUNDIAI — lógica do projeto
...
const COL_LOCAIS   = () => db.collection('ronda-jundiai-locais');
...
const COL_RONDAS   = () => db.collection('ronda-jundiai-rondas');
const COL_LIXEIRA  = () => db.collection('lixeira-ronda-jundiai');
const COL_LOGS     = () => db.collection('ronda-jundiai-logs');
...
async function iniciarApp() {
  ...
  registrarLog('acesso', 'Entrou no projeto', 'Acessou a página da Ronda Callink Jundiaí.');
  irPara('dashboard');
}
...
async function carregarTecnicos() {
  ...
  _tecnicos = s.docs.map(d => ({ id: d.id, ...d.data() }))
    .filter(u => !u.rondaJundiaiCliente && (u.role === 'admin' || u.role === 'superadmin' || (u.projects && u.projects['ronda-jundiai'])))
...
async function renderClientes(root) {
  ...
  try {
    ...
    const usuarios = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .filter(u => u.role === 'user' && (u.rondaJundiaiCliente || (u.projects && u.projects['ronda-jundiai'])))
      .sort((a, b) => (a.name || a.email || '').localeCompare(b.name || b.email || ''));
    ...
    box.innerHTML = '<div class="list">' + usuarios.map(u => {
      const vinculados = new Set(Array.isArray(u.rondaJundiaiLocais) ? u.rondaJundiaiLocais : []);
      ...
      return `
        <div class="card" id="cli_${u.id}">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
            ...
            <label class="checkline"><input type="checkbox" id="ehCli_${u.id}" ${u.rondaJundiaiCliente ? 'checked' : ''}> É cliente externo (somente leitura)</label>
          </div>
...
async function salvarCliente(uid) {
  ...
  try {
    await db.collection('users').doc(uid).update({ rondaJundiaiCliente: ehCliente, rondaJundiaiLocais: locais });
...
async function criarCliente() {
  ...
  try {
    ...
    await db.collection('users').doc(uid).set({
      ...
      projects: { 'ronda-jundiai': true },
      rondaJundiaiCliente: true,
      rondaJundiaiLocais: locais,
      criadoPor: window._userEmail || '',
...
async function renderLogs(root) {
  ...
  root.innerHTML = `
    <div class="view-header">
      <div class="view-title"><h2><i class="fas fa-list-alt"></i> Logs</h2><p>Histórico de acessos, navegação e ações realizadas na Ronda Callink Jundiaí.</p></div>
      <div class="view-actions">
