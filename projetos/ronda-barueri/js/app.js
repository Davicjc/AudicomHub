/* ================================================================
   RONDA CALLINK BARUERI — lógica do projeto
...
const COL_LOGS     = () => db.collection('ronda-barueri-logs');
...
async function iniciarApp() {
  ...
  registrarLog('acesso', 'Entrou no projeto', 'Acessou a página da Ronda Callink Barueri.');
  irPara('dashboard');
}
...
async function carregarTecnicos() {
  ...
  _tecnicos = s.docs.map(d => ({ id: d.id, ...d.data() }))
    .filter(u => !u.rondaBarueriCliente && (u.role === 'admin' || u.role === 'superadmin' || (u.projects && u.projects['ronda-barueri'])))
...
async function renderClientes(root) {
  ...
  try {
    ...
    const usuarios = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      .filter(u => u.role === 'user' && (u.rondaBarueriCliente || (u.projects && u.projects['ronda-barueri'])))
      .sort((a, b) => (a.name || a.email || '').localeCompare(b.name || b.email || ''));
    ...
    box.innerHTML = '<div class="list">' + usuarios.map(u => {
      const vinculados = new Set(Array.isArray(u.rondaBarueriLocais) ? u.rondaBarueriLocais : []);
      ...
      return `
        <div class="card" id="cli_${u.id}">
          <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap">
            ...
            <label class="checkline"><input type="checkbox" id="ehCli_${u.id}" ${u.rondaBarueriCliente ? 'checked' : ''}> É cliente externo (somente leitura)</label>
          </div>
...
async function salvarCliente(uid) {
  ...
  try {
    await db.collection('users').doc(uid).update({ rondaBarueriCliente: ehCliente, rondaBarueriLocais: locais });
...
async function criarCliente() {
  ...
  try {
    ...
    await db.collection('users').doc(uid).set({
      ...
      projects: { 'ronda-barueri': true },
      rondaBarueriCliente: true,
      rondaBarueriLocais: locais,
      criadoPor: window._userEmail || '',
...
async function renderLogs(root) {
  ...
  root.innerHTML = `
    <div class="view-header">
      <div class="view-title"><h2><i class="fas fa-list-alt"></i> Logs</h2><p>Histórico de acessos, navegação e ações realizadas na Ronda Callink Barueri.</p></div>
      <div class="view-actions">
